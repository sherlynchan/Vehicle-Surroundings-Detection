...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('models_pkl/roadmap_model_epoch7.pkl'))


    criterion = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    optimizer.load_state_dict(torch.load('models_pkl/roadmap_optimizer_epoch7.pkl'))
    print('2')
    #torch.save(model.state_dict(), "models_pkl/roadmap_model_epoch" + str(0) + ".pkl")
    #torch.save(optimizer.state_dict(), "models_pkl/roadmap_optimizer_epoch" + str(0) + ".pkl")
    for ep in range(3, 30):
        model.train()
        ...
        for it, (sample, target, road_image, extra) in enumerate(trainloader):
            ...
            pred = model(sample)
            labels = (road_image[0].reshape(-1,800*800)+0).float().to(device)
            ...
            yl = yl + loss
        torch.save(model.state_dict(), "models_pkl/roadmap_model_epoch" + str(ep + 1) + ".pkl")
        torch.save(optimizer.state_dict(), "models_pkl/roadmap_optimizer_epoch" + str(ep + 1) + ".pkl")
