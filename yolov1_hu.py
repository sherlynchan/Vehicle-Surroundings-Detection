                ...
                for n in range(7):  # y方向网格循环
                    if labels[i, 5, m, n] == 1:  # 如果包含物体
                        ...
                        bbox1_pred_xyxy = ((pred[i, 0, m, n] + m) / num_gridx - pred[i, 2, m, n] / 2,
                                           ...
                                           (pred[i, 1, m, n] + n) / num_gridy + pred[i, 3, m, n] / 2)
                        bbox2_pred_xyxy = ((pred[i, 6, m, n] + m) / num_gridx - pred[i, 8, m, n] / 2,
                                           (pred[i, 7, m, n] + n) / num_gridy - pred[i, 9, m, n] / 2,
                                           (pred[i, 6, m, n] + m) / num_gridx + pred[i, 8, m, n] / 2,
                                           (pred[i, 7, m, n] + n) / num_gridy + pred[i, 9, m, n] / 2)
                        ...
                        if iou1 >= iou2:
                            ...
                            coor_loss = coor_loss + 5 * (torch.sum((pred[i, 6:8, m, n] - labels[i, 6:8, m, n]) ** 2) \
                                                         + torch.sum(
                                        (pred[i, 8:10, m, n].sqrt() - labels[i, 8:10, m, n].sqrt()) ** 2)) \
